...
def debug_print_maps():
        ...
        for name, filename in map_files.items():
            try:
                tmx_data = load_pygame(join('data/maps', filename))
                maps[name] = tmx_data
                print(f"Loaded map '{name}' successfully.")
    ...
    def setup(self, tmx_map, player_start_pos):
        ...
        for obj in tmx_map.get_layer_by_name('Objects'):
            if obj.name == 'top':
                ...
                CollidableSprite((obj.x, obj.y), obj.image, (self.all_sprites, self.collision_sprites), WORLD_LAYERS['main'])
        ...
        for obj in tmx_map.get_layer_by_name('Entities'):
            ...
            elif obj.properties.get('entity') == 'npc':
                Character(
                    pos=(obj.x, obj.y),

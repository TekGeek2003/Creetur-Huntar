...
def outline_creator(frame_dict, padding=4):
	...
	for monster, monster_frames in frame_dict.items():
		...
		for state, frames in monster_frames.items():
			...
			for frame in frames:
				size = pygame.Vector2(frame.get_size()) + pygame.Vector2(padding * 2)
				...
				# Black outline (outer border)
				black_outline = mask.to_surface(
					setcolor=(0, 0, 0, 255),
					unsetcolor=(0, 0, 0, 0)
				)

				# Blit white silhouette first (center)
				surf.blit(white_sil, (padding, padding))
				# Blit black outlint offset by 1 pixel to create a solid border
				for x in range(-1, 2):
					for y in range(-1, 2):
						if x == 0 and y == 0:
							continue
						surf.blit(black_outline, (padding + x, padding + y))
